// cyclopedia-app/src/js/human-body.js
...
async function loadHumanBody() {
  const container = document.querySelector(".human-body-card");
  container.innerHTML = "";

  const querySnapshot = await getDocs(collection(db, "categories", "human-body", "items"));

  querySnapshot.forEach((doc) => {
    const data = doc.data();
    const name = data.name?.[currentLang] || "؟";
    const image = `/images/human-body/${data.image}`;
    const sound = `/audio/${currentLang}/human-body/${data.voices?.teacher}`;
    const description = data.description?.[currentLang] || "";

    container.innerHTML += `
      <div class="human-body-entry card">
        <h3>${name}</h3>
