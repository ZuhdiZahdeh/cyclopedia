...
async function loadFish() {
  const container = document.querySelector(".fish-card");
  container.innerHTML = "";

  const querySnapshot = await getDocs(collection(db, "categories", "fish", "items"));

  querySnapshot.forEach((doc) => {
    const data = doc.data();
    const name = data.name?.[currentLang] || "؟";
    const image = `/images/fish/${data.image}`;
    const sound = `/audio/${currentLang}/fish/${data.voices?.teacher}`;
    const description = data.description?.[currentLang] || "";

    container.innerHTML += `
      <div class="fish-entry card">
        <h3>${name}</h3>
