// cyclopedia-app/src/js/plants.js
...
async function loadPlants() {
  ...
  const querySnapshot = await getDocs(collection(db, "categories", "plants", "items"));

  querySnapshot.forEach((doc) => {
    const data = doc.data();
    const name = data.name?.[currentLang] || "؟";
    const image = `/images/plants/${data.image}`;
    const sound = `/audio/${currentLang}/plants/${data.voices?.teacher}`;
    const description = data.description?.[currentLang] || "";

    container.innerHTML += `
      <div class="plant-entry card">
        <h3>${name}</h3>
