// cyclopedia-app/src/js/transport.js
...
async function loadTransport() {
  const container = document.querySelector(".transport-card");
  container.innerHTML = "";

  const querySnapshot = await getDocs(collection(db, "categories", "transport", "items"));

  querySnapshot.forEach((doc) => {
    const data = doc.data();
    const name = data.name?.[currentLang] || "؟";
    const image = `/images/transport/${data.image}`;
    const sound = `/audio/${currentLang}/transport/${data.voices?.teacher}`;
    const description = data.description?.[currentLang] || "";

    container.innerHTML += `
      <div class="transport-entry card">
        <h3>${name}</h3>
