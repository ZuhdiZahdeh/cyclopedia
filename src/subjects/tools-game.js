...
function setImageWithFallback(imgEl, candidates){
  let i=0;
  const tryNext=()=>{ if(!imgEl) return; if(i>=candidates.length){ imgEl.src='/images/default.png'; return; }
...
function clearCarousel(){
  ...
  area.querySelector('#tool-carousel-thumbs')?.remove();
  area.querySelector('#tool-carousel-prev')?.remove();
  area.querySelector('#tool-carousel-next')?.remove();
}
function buildCarousel(displayName){
  ...
  currentToolImages.forEach((src,idx)=>{ const t=document.createElement('img'); t.src=src; t.alt=displayName||''; t.className='carousel-thumb';
    t.onclick=()=>{ currentImageIndex=idx; mainImg.src=currentToolImages[currentImageIndex]; syncThumbsActive(); }; thumbs.appendChild(t); });
...
function updateToolContent(){
  ...
  if (!tools.length){
    pick('tool-word','tool-name')?.textContent='—';
    const img=pick('tool-image'); if(img){ img.removeAttribute('src'); img.alt=''; }
    pick('tool-description')?.textContent='—';
    pick('tool-professions')?.textContent='—';
    clearCarousel();
    return;
  }
  ...
  if (imgEl){
    ...
    imgEl.alt=displayName||''; imgEl.classList.add('clickable-image'); imgEl.onclick=playCurrentToolAudio;
  }
  ...
  const nextBtn=grab(['next-tools-btn','next-btn']); const prevBtn=grab(['prev-tools-btn','prev-btn']);
...
/* ============== تنقّل وصوت ============== */
export function showNextTool(){ if(!tools.length) return; if(currentIndex<tools.length-1) currentIndex++; updateToolContent();
  try{ const u=JSON.parse(localStorage.getItem('user')); if(u) recordActivity(u,'tools'); }catch{} }
export function showPreviousTool(){ if(!tools.length) return; if(currentIndex>0) currentIndex--; updateToolContent();
  try{ const u=JSON.parse(localStorage.getItem('user')); if(u) recordActivity(u,'tools'); }catch{} }
...
async function ensureToolsSidebar(){
  ...
  let html='';
  try{ const resp=await fetch('/html/tools-controls.html',{cache:'no-store'}); if(resp.ok) html=await resp.text(); }catch(e){ console.warn('[tools] controls fetch error:',e); }
  const tmp=document.createElement('div'); tmp.innerHTML=(html||FALLBACK_HTML).trim();
  container=tmp.firstElementChild; container.id='tools-sidebar-controls'; container.classList.add('subject-controls'); container.style.display='block';
...
export async function loadToolsGameContent(){
  console.log('[tools] loadToolsGameContent()'); stopCurrentAudio();

  const main=document.querySelector('main.main-content'); if(!main){ console.error('main.main-content not found'); return; }
  try{ const resp=await fetch('/html/tools.html',{cache:'no-store'}); main.innerHTML=await resp.text(); }catch{
    main.innerHTML=`<section id="tools-game" class="topic-container subject-page"><div class="game-box">
      ...
      <div class="tool-description-box info-box" id="tool-description-box" style="display:none;">
        <h4 data-i18n="common.description">الوصف</h4><p id="tool-description">—</p>
        <p><strong data-i18n="tools.related_professions">المهن المرتبطة</strong>: <span id="tool-professions">—</span></p>
      </div></div></section>`;
  }

  ensureCss('/css/tools.css','tools-css');  // ← مهم

  const controls=await ensureToolsSidebar(); if(!controls){ console.warn('[tools] controls not mounted'); return; }
  ...
  if (toggleDescBtn){
    toggleDescBtn.onclick=()=>{ const box=document.getElementById('tool-description-box')||document.querySelector('#tools-game .info-box');
      if(box) box.style.display=(box.style.display==='none'?'block':'none'); };
  }
  if (langSelect){
    try{ langSelect.value=getCurrentLang(); }catch{}
    langSelect.onchange=async()=>{ const lng=langSelect.value; await loadLanguage(lng); setDirection(lng); applyTranslations(); updateToolContent(); };
  }
  if (voiceSelect && !voiceSelect.value) voiceSelect.value='teacher';

  tools=[]; if(prevBtn) prevBtn.disabled=true; if(nextBtn) nextBtn.disabled=true; if(playSoundBtn) playSoundBtn.disabled=true;

  await fetchTools();
  if (!tools.length){
    pick('tool-word','tool-name')?.textContent='لا توجد بيانات';
    const img=pick('tool-image'); if(img) img.src='/images/default.png';
    pick('tool-description')?.textContent='—'; pick('tool-professions')?.textContent='—'; clearCarousel();
    return;
  }

  const lang=getCurrentLang(); tools.sort((a,b)=> (a?.name?.[lang]||'').localeCompare(b?.name?.[lang]||''));
  currentIndex=0; updateToolContent();
  ...
  if (typeof window!=='undefined'){
    window.loadToolsGameContent=loadToolsGameContent; window.showNextTool=showNextTool; window.showPreviousTool=showPreviousTool; window.playCurrentToolAudio=playCurrentToolAudio;
  }
