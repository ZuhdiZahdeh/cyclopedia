...
// المهن — اسم يتبدّل مع اللغة + ربط IDs مرن + مسارات صوت احتياطية
...
function updateProfessionContent(){
  ...
  if(!professions.length){
    { const el = pick('profession-word','item-word','item-name'); if (el) el.textContent = '—'; }
  ...
  const wordEl=pick('profession-word','item-word','item-name');
  const imgEl =pick('profession-image','item-image');

  if (wordEl){ setHighlightedName(wordEl,name); wordEl.classList.add('clickable-text'); wordEl.onclick=playCurrentProfessionAudio; }
  if (imgEl){
    const src=(d.image_path||d.image||'');
    imgEl.src = (isAbs(src)||src.startsWith('/')) ? src : `/images/professions/${norm(src)}`;
    imgEl.alt = name || '';
...
export function showNextProfession(){ if(!professions.length) return; if(currentIndex<professions.length-1) currentIndex++; updateProfessionContent();
  try{ const u=JSON.parse(localStorage.getItem('user')); if(u) Promise.resolve(recordActivity(u,'professions')).catch(()=>{}); }catch{} }
...
export async function loadProfessionsGameContent(){
  ...
  if (langSelect){
    ...
    langSelect.onchange=async()=>{ const lng=langSelect.value; await loadLanguage(lng); setDirection(lng); applyTranslations(); updateProfessionContent(); };
  }
  ...
  if (!professions.length){
    { const el = pick('profession-word','item-word','item-name'); if (el) el.textContent = 'لا توجد بيانات'; }
