...
function createBoard() {
    ...
    shuffledCards.forEach(card => {
        ...
        if (card.type === 'image') {
            ...
            // ✅ التعديل هنا: إذا كانت البطاقة من نوع 'image'، لا تعرض النص أسفل الصورة
            cardText = ''; // اجعل النص فارغًا لهذه الحالة
        ...
        cardElement.innerHTML = `
            <div class="front-face">
                ${frontContent}
                ${card.type !== 'image' ? `<span class="card-text">${cardText}</span>` : ''} </div>
...
function updateCardTexts() {
    cards.forEach(cardElement => {
        ...
        if (originalItem) {
            // ✅ التعديل هنا: فقط قم بتحديث النص إذا لم تكن البطاقة من نوع 'image'
            if (cardType !== 'image') {
