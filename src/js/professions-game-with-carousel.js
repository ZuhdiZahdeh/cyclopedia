...
function updateProfessionContent() {
  ...
  const imageContainer = document.getElementById("profession-image");

  // Clear old content if carousel
  ...
  if (currentProfessionData.images && currentProfessionData.images.length > 1) {
    const container = document.createElement("div");
    container.className = "carousel-container";

    const img = document.createElement("img");
    img.id = "carousel-image";
    img.src = `/${currentProfessionData.images[0]}`;
    ...
    let index = 0;
    prevBtn.onclick = () => {
      index = (index - 1 + currentProfessionData.images.length) % currentProfessionData.images.length;
      img.src = `/${currentProfessionData.images[index]}`;
    };
    nextBtn.onclick = () => {
      index = (index + 1) % currentProfessionData.images.length;
      img.src = `/${currentProfessionData.images[index]}`;
    };
    ...
    parent.insertBefore(container, parent.children[1]);

  } else {
    // صورة واحدة فقط
    const img = document.createElement("img");
    img.id = "profession-image";
